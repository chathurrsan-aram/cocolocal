...
## Shop explorer and private wheel — 18 September
- Added a shop-only explorer using the existing approved five-unit 3D geometry, with five selectable customer areas and matching real retail-floor photos. Approximate-layout wording retained; no private property guide or ancillary-room photographs included.
- Added a homepage button and password-protected `/spin` preview, explicitly noindex and outside the sitemap. The entire demo HTML is AES-256-GCM encrypted using a high-entropy password, stored only in a private access file outside this repository. Password is required again after page reload.
- Owner selected a browser-only demo and confirmed both coffee and slushies are available. Six equal segments: two slushies, two coffees, two try-again-tomorrow outcomes. No purchase/payment or actual redeemable prize; odds shown.
- Daily browser limit is enforced with an atomic IndexedDB read/write transaction keyed to the Europe/London calendar day. Results persist through refresh and duplicate tabs. It is not person-level identity enforcement: clearing site data, another browser/device/origin or clock manipulation can bypass a browser-only demo.
- Custom vector artwork, reduced-motion handling, animated deceleration, saved-result panel and responsive iframe height. Wheel artwork reviewed as a rendered SVG asset; full browser interaction review remains unavailable.
- Unit checks passed with fake-indexeddb: 16 simultaneous attempts over two database connections produce exactly one new result, saved-result persistence, next-day eligibility, summer/winter UK-midnight boundaries, segment bounds, correct-password decryption and wrong-password/tamper rejection. No password found in published source/assets/docs.
- Private editable source: sibling `outputs/Wheel-Preview/wheel.html`; private password file alongside it. Re-encrypt with `node scripts/encrypt-wheel.mjs SOURCE_HTML ACCESS_JSON` from this repository after changes. Never commit the private access file or decrypted demo HTML.
- Customer launch still requires real identity/abuse controls, prize inventory, final odds/quantities, redemption verification and approved promotion terms. This implementation is the requested private browser-only demo.

### Wheel and offers refinement — 18 September 2026
- Enlarged private wheel: full column width, wider desktop allocation, up to 600px on stacked layouts. Moved labels toward rim and used existing Coco basket logo in centre.
